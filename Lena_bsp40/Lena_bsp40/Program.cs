...
class PgmCreatorDemo {
	...
	/* * Übungsfunktionen für Übung 7 * */

	// Horizontal
	static void HorizontalSpiegeln()
	{
		byte[,] lenaImgData;
		byte[,] orgData;
		try
		{
			orgData = PgmCreator.ReadPgmFile("/Users/danielraudschus/Documents/ImageProcessing-PGM-Files/lena.pgm");
			lenaImgData = PgmCreator.ReadPgmFile("/Users/danielraudschus/Documents/ImageProcessing-PGM-Files/lena.pgm");
			for (int x = 0; x < lenaImgData.GetLength(0); x++)
			{

				for (int y = 0; y < lenaImgData.GetLength(1); y++)
				{
					lenaImgData[x,lenaImgData.GetLength(0)-1-y] = orgData[x, y];
				}
			}
			PgmCreator.WritePgmFile("/Users/danielraudschus/Documents/ImageProcessing-PGM-Files/lena_kopfstehen.pgm", lenaImgData, PgmType.P5);
			Console.WriteLine("Success, this turns lena upside down!");
		}
		catch (Exception e)
		{
			Console.WriteLine(e.Message);
	...
	static void VertikalSpiegeln()
	{
		...
		try
		{
			...
			Console.WriteLine("Nice mirror effect");
		}
		catch (Exception e)
		{
			Console.WriteLine(e.Message);
	...
	// Lena 90° nach links rotieren lassen
	static void Rotieren90Links()
	{
		byte[,] lenaImgData; // 2D Array anlegen
		byte[,] orgData; // 2D Array anlegen
		try
		{
			orgData = PgmCreator.ReadPgmFile("/Users/danielraudschus/Documents/ImageProcessing-PGM-Files/lena.pgm");
			lenaImgData = PgmCreator.ReadPgmFile("/Users/danielraudschus/Documents/ImageProcessing-PGM-Files/lena.pgm");
			...
			Console.WriteLine("Success, this turns lena left!");
		}
		catch (Exception e)
		{
			Console.WriteLine(e.Message);
	...
	// Aufgabe a) schwarzer Balken über Lenas Augen
	static void Black()
	{
		byte[,] lenaImgData;
		try {
			lenaImgData = PgmCreator.ReadPgmFile("/Users/danielraudschus/Documents/ImageProcessing-PGM-Files/lena.pgm");
			for (int x = 225; x < 375; x++) {
				for (int y = 250; y < 300; y++) {
					lenaImgData[x, y] = 0;
				}
			}
			PgmCreator.WritePgmFile("/Users/danielraudschus/Documents/ImageProcessing-PGM-Files/lena_black.pgm", lenaImgData, PgmType.P5);
			Console.WriteLine("Success, this puts a black sign over lenas eyes");
		} catch (Exception e) {
			Console.WriteLine(e.Message);
		}
	}
